...
const ShortenItem = ({ originalUrl, shortUrl, clickCount, createdDate, showAnalytics=true }) => {
    ...
    const subDomain = import.meta.env.VITE_REACT_SUBSOMAIN.replace(
        /^https?:\/\//,
        ""
    );

    const analyticsHandler = ( shortUrl ) => {
        console.log('analyticsHandler: ', shortUrl);
        ...
        <div className={`flex sm:flex-row flex-col  sm:justify-between w-full sm:gap-0 gap-5 py-5 `}>
            <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
                <div className="text-slate-900 pb-1 sm:pb-0 flex items-center gap-2 ">
                    <Link
                        ...
                        to={import.meta.env.VITE_REACT_SUBSOMAIN + "/" + `${shortUrl}`}>
                            {subDomain + `/${shortUrl}`}
                    </Link>
            ...
            <div className="flex  flex-1  sm:justify-end items-center gap-4">
                <CopyToClipboard
                    ...
                    text={`${import.meta.env.VITE_REACT_SUBSOMAIN + "/" + `${shortUrl}`}`}
                >
